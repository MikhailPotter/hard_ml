...
def average_precision(ys_true: torch.Tensor, ys_pred: torch.Tensor) -> float:
    ...
    order = ys_pred.argsort(descending=True)
    ys_true_cumsum = ys_true[order].cumsum(dim=0)
    precision = (ys_true_cumsum / torch.arange(1, ys_true_cumsum.size(0) + 1)).tolist()
    recall = (ys_true_cumsum / ys_true.sum().item()).tolist()
    ap = 0.0
    recall_ = 0.0
    for i in range(len(precision)):
        ap += (recall[i] - recall_) * precision[i]
        recall_ = recall[i]
    return ap
